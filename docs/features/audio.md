...
**Live-verified on 2026-09-19**: run 1 established ADSP boot, card registration and mmap playback with manual steps; run 2 (`2076c85f…`) verified the shipped scripts end to end — `audio-up` reported the card 1.59 s after `boot_adsp` and stayed resident, `speaker-test-tone -s`, `-v 15` and `-p -d 3 -v 15` all ran clean (mmap `RUNNING`, route back to `Off`, protection attempt logged as best-effort), and `gps-up` then booted the modem on the shared `/firmware` mount and locator (`modem up`, crash_count 0). The user heard every tone cleanly. See [next steps](../next-steps/connectivity-and-sensors.md#audio) for the remaining work and the [build log](../build-log.md) for the dated entries.
...
Expected, harmless kernel-log noise on every stream open (no ACDB loader, by design): `afe_get_cal_topology_id: [AFE_TOPOLOGY_CAL] not initialized for this port 4100`, `send_afe_cal_type cal_block not found!!`, `q6asm_send_cal: cal_block is NULL`, `q6core_send_get_avcs_fwk_ver_cmd: DSP returned error[ADSP_EUNSUPPORTED]`, and a few `q6asm_callback: payload size of 8 is less than expected.` (the ADSP answers some ASM commands with an 8-byte basic response; the kernel logs it and carries on — ~4 per mmap play, ~136 per non-mmap XRUN-looping play). `tinymix contents` also triggers ~10 `TAS2560_ALGO:… get param` lines.
...
### The protection algorithm is unreachable for now
...
`speaker-test-tone` therefore leaves the control alone by default. `-p` makes a **best-effort, never-fatal** attempt 1 s into playback from a background job and logs the set rc and the read-back value, so the behaviour can be re-checked once calibration/port sequencing is worked out ([next steps](../next-steps/connectivity-and-sensors.md#audio)). What actually protects the uncalibrated speaker meanwhile is the amplitude ceiling: `speaker-test-tone` defaults to **-20 dBFS** and clamps `-a` to **-6 dBFS**, and `wavtone` independently refuses (does not clamp) anything louder than -6 dBFS (`dbfs_to_amplitude`'s `-1` sentinel). Duration defaults to 1 s, clamped to 5 s. `DAC Playback Volume` defaults to 10; stock's initial mixer setup leaves it at 15 and the live run used 15 at -20 dBFS — `-v 15` reproduces that.

The stock images contain a plausible minimal calibration path that does not require importing the full Android audio stack. `vendor_a` contains `Speaker_cal.acdb` and `libacdbloader.so`; the latter depends on Bionic/Android libraries and `/dev/msm_audio_cal`, so it is not directly usable in this musl initramfs. More specifically for the TAS2560, the stock 32-bit `audio.primary.sdm660.so` reads `/mnt/vendor/persist/factory/audio/tas2560_calib_rdc`, converts the decimal resistance to Q19 (`Rdc * 524288`), and sends it through the five-value `TAS2560_ALGO_CMD_SEND_CAL` mixer control before enabling the speaker path. This device's read-only `persist` dump contains `Rdc 7.081085`, `F0 923.762394`, and `Q 1.467016`; its Rdc converts to `3712528`. The kernel implements Rdc start/stop/read/send controls, while its F0/Q calibration start/stop cases explicitly say `Not Implemented`. A future live probe can therefore mount `persist` read-only, parse the device-specific Rdc, send the Q19 value while the RX/TX ports are active, and check whether `TAS2560_ALGO_FF_MODULE` then remains enabled. Do not replace that per-device value with a generic constant.
...
speaker-test-tone -p                   # same as default plus the best-effort protection attempt 1 s in (logs rc + read-back)
```
...
- `tools/tests/test_speaker-test-tone.sh` (48 checks): argument clamping (`clamp_duration`, `clamp_amplitude`) and the same FIFO/no-`-s` `card_ready` checks via the sourcing hook, plus full runs of the real script against a fake `/proc/asound` tree with stub `tinymix`/`tinyplay`/`wavtone` on `PATH` recording their argv — the exact `tinymix` call sequence (route, volume; protection untouched by default), `tinyplay … -D 0 -d 0 -M`, `-p` attempting `TAS2560_ALGO_FF_MODULE` only after `tinyplay` has started and logging rc + read-back, `-p` still exiting 0 with tinymix's own `Error: invalid enum value` surfaced in the log when the set is rejected, the old `-P` rejected, `-s` printing card/device/three `tinymix get` values and nothing else (also without a card), the mixer route reset to 0 on exit even when `tinyplay` fails, refusal with zero side effects without the card, and the amplitude clamp reaching `wavtone` already capped at -6 dBFS.
...
What these tests **cannot** cover without a device: `mountpoint -q /firmware` real-filesystem behavior (both scripts), the real `servreg-locator`/`irsc` subprocesses, the ADSP boot itself, and the audible result. Run 2 covered the shipped scripts end to end, including coexistence with `gps-up`, clean mmap playback and route cleanup; see the current-status section and build log.

Rebuild the baseline initramfs and boot image after any change here (`scripts/mkinitramfs.sh` fails closed if `tools/wavtone.c` is missing or if the rootfs lacks `tinymix`/`tinyplay` — rerun `scripts/mkrootfs.sh` first if so). Live checks: `audio-up` logs `adsp up … after boot_adsp` within a couple of seconds (1.17 s measured) and stays resident when it started the locator, `speaker-test-tone -s` shows the card/device/controls, `speaker-test-tone` is audible and clean at the default settings with `state: RUNNING` and 0 xruns, `-p`'s log line shows the set rc and read-back, `-v`/`-a`/`-d`/`-f` behave as documented, and the mixer route is left at 0 afterward. Record image hashes and the live evidence in the [build log](../build-log.md).
