...
Audio on this SoC runs on the ADSP (Hexagon DSP), not the application processor. Unlike GPS's modem, the ADSP is never torn down once booted — there is no unload path — so `initramfs/usr/bin/audio-up` is a one-shot bring-up, not a supervised lifecycle like `gps-up`. **Reboot is the only way to power the ADSP back down.**
...
There is no way to power the ADSP down short of a reboot — `audio-up` never tears it down, by design (see "Why audio needs its own subsystem bring-up" above).
...
What these tests **cannot** cover without a device: `mountpoint -q /firmware` real-filesystem behavior (both scripts), the real `servreg-locator`/`irsc` subprocesses, the ADSP boot itself, and the audible result. Run 1 covered the manual equivalents of all of those (above); the scripts themselves are pending re-verification after the fixes.
