...
- **Speaker protection (`TAS2560_ALGO_FF_MODULE`)**: unreachable from the current scripts. Observed live: `tinymix set … ENABLE` (or `DISABLE`, or by index) returns `Error: invalid enum value` whenever the `TERT_MI2S_RX` AFE port is inactive; set 1 s into a running stream it returns rc=0 but reads straight back as 0. Offline inspection found the missing device-specific path: stock `persist` holds `tas2560_calib_rdc` (`7.081085` on this phone), and the stock HAL converts it to Q19 (`3712528`) and sends it through `TAS2560_ALGO_CMD_SEND_CAL` before enabling the path. Prototype that small read-only loader and port-active sequence rather than importing Android's Bionic-dependent ACDB stack. Keep the conservative amplitude ceiling until it is live-verified.
