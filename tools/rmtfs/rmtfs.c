...
static void rmtfs_iovec(int sock, struct qrtr_packet *pkt)
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		for (j = 0; j < entries[i].num_sector; j++) {
			...
			if (n != SECTOR_SIZE) {
				fprintf(stderr, "[RMTFS] failed to %s sector %u\n",
					is_write ? "write" : "read",
					entries[i].sector_addr + (uint32_t)j);
				qmi_result_error(&resp.result, QMI_RMTFS_ERR_INTERNAL);
				goto respond;
			}
...
static int handle_rmtfs(int sock)
{
	...
	/*
	 * AF_MSM_IPC has no wire-compatible sockaddr_qrtr, so the bridge
	 * (tools/msmipc.c) hands back node/port instead of filling one; build
	 * the sockaddr_qrtr locally for qrtr_decode(). qrtr_recvfrom() returns
	 * QRTR_RECV_RESUME_TX for the kernel's standalone RESUME_TX flow-control notice: it is
	 * neither EOF nor a packet, so return to select() without decoding it.
	 */
	ret = qrtr_recvfrom(sock, buf, sizeof(buf), &node, &port);
	if (ret == QRTR_RECV_RESUME_TX || rmtfs_recv_is_retryable_error(ret))
		return 0;
	if (ret < 0) {
		if (!rmtfs_recv_requires_rebind(ret))
...
static int run_rmtfs(void)
{
	...
	for (;;) {
		...
		if (FD_ISSET(rmtfs_fd, &rfds)) {
			...
			if (rmtfs_recv_requires_rebind(ret))
				break;
			/* RESUME_TX and retryable receive errors are handled as a normal empty
			 * poll cycle by handle_rmtfs(); only a subsystem reset restarts
			 * this server. Other decode/I/O errors are logged by the handler
			 * and leave the service registered for the next request. */
		}
	}
...
int main(int argc, char **argv)
{
	...
	while ((option = getopt(argc, argv, "o:S:Prv")) != -1) {
		switch (option) {
		/*
	...
	do {
		ret = run_rmtfs();
	} while (rmtfs_recv_requires_rebind(ret));
