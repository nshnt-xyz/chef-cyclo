...
int storage_init(const char *storage_root, bool read_only, bool use_partitions)
{
	...
	/* Assigned unconditionally (not just set-if-true) so a process that
	 * re-initializes storage doesn't keep the previous call's partition
	 * mode; rmtfs itself only calls this once, but tests do it per case. */
	storage_use_partitions = use_partitions;
...
struct rmtfd *storage_get(unsigned node, int caller_id)
{
	...
	if (caller_id < 0 || caller_id >= MAX_CALLERS)
		return NULL;

	rmtfd = &rmtfds[caller_id];
	if (!rmtfd->partition || rmtfd->node != node)
		return NULL;
...
ssize_t storage_pwrite(struct rmtfd *rmtfd, const void *buf, size_t nbyte, off_t offset)
{
	...
	if (offset < 0 || (uintmax_t)offset > STORAGE_MAX_SIZE ||
	    nbyte > STORAGE_MAX_SIZE - (size_t)offset) {
		fprintf(stderr, "write exceeds the %u-byte RAM shadow limit\n",
			STORAGE_MAX_SIZE);
		errno = EFBIG;
		return -1;
	}
...
static int storage_populate_shadow_buf(struct rmtfd *rmtfd, const char *file)
{
	...
	while (done < (size_t)len) {
		n = read(fd, (char *)buf + done, (size_t)len - done);
		if (n < 0) {
			if (errno == EINTR)
				continue;
			ret = -1;
